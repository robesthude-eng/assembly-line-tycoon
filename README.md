...
* `FactoryBuilder` — постройка, улучшение, прокладка и разворот ленты, снос; цены из `MachineCatalog` и `BeltCatalog`
* Диалог ленты (развернуть/снести) и кнопка сноса в карточке машины
* `GameState.NEW_GAME` — стартовый капитал 200 монет: ровно на минимальную работающую линию
* Автосохранение раз в 30 с + сохранение при уходе с экрана
* Офлайн-доход за время отсутствия с потолком в 2 часа и окном итогов при запуске
...
| ~~4c. Редактор~~ | ✅ Прокладка и разворот конвейера, снос, стартовый капитал; цена ленты внесена в баланс (`BeltCatalog`) |
