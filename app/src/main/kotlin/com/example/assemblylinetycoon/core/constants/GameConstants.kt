...
object GameConstants {
    ...
    /**
     * Сколько предмет едет по одной ячейке ленты (мс).
     *
     * Это не экономический параметр, а скорость анимации: доход задаётся
     * тактами машин, лента лишь доставляет. Значение подобрано так, чтобы
     * движение читалось глазом — 2 клетки в секунду.
     */
    const val BELT_TRAVEL_TIME_MS: Long = 500L
    ...
    /**
     * Стартовый капитал новой игры.
     *
     * Подобран не «на глаз», а под минимальную работающую линию: карьер (50) +
     * экспортёр (100) + четыре отрезка ленты между ними (~40) = 190 монет.
     * Меньше — и первый экран игры оказался бы тупиком: строить не на что,
     * а зарабатывать нечем. Проверяется тестом по каталогам, а не константой.
     */
    const val STARTING_COINS: Long = 200L

    /** Версия схемы сохранения. Инкрементируется при несовместимых изменениях. */
    const val SAVE_SCHEMA_VERSION: Int = 1

    /** Предохранитель от «прыжка» симуляции после долгой паузы (мс). */
